...
print('check license header on source files...')
...
def CheckDirFiles(path):
    if os.path.isfile(path):
        file = open(path, 'r')
        oldFile = file.read()
        if not oldFile.startswith(licenseNotice):
            file.close()
            file = open(path, 'w')
            print('insert license to: %s' % path)
            file.write('%s%s' % (licenseNotice, oldFile))
        return
    workingDir = os.listdir(path)
    for dir in workingDir:
        CheckDirFiles('%s/%s' % (path, dir))

CheckDirFiles('%sSource' % unrealFolder)
